...
# 🩺 MedAI- Medical Chatbot (RAG-based Clinical Reference Assistant)

Context-aware Q&A over a local medical knowledge base using FAISS vector search + Groq-hosted LLMs.

</div>

## 🧠 Overview
This project implements a Retrieval-Augmented Generation (RAG) pipeline that lets you ask medical questions grounded in a curated PDF knowledge base (e.g. an encyclopedia of medicine). The LLM is constrained by retrieved passages from a FAISS vector store built from your documents.

Two primary entry points:
- `connect_memory_with_llm.py` – CLI prototype using a HuggingFace Inference endpoint (e.g. Mistral 7B Instruct).
- `medAI.py` – Streamlit chat UI using a Groq-hosted model (Llama 4 Maverick) with retrieval.
...
| `medAI.py` | Streamlit chat interface using Groq Chat model + FAISS retrieval |
...
streamlit run medAI.py                   # chat UI (needs GROQ_API_KEY)
...
For local development, keep secrets out of version control and use a `.env` file that is ignored by Git. A small example file is provided as `.env.example` — copy it to `.env` and fill in real values.

Quick steps (zsh):

```zsh
cd /path/to/medical-chatbot-refactored
python3 -m venv .venv               # create venv (one time)
...
cp .env.example .env
# edit .env and fill HF_TOKEN and GROQ_API_KEY

# load vars into shell (exports all KEY=VALUE pairs)
...
Tip: add `TOKENIZERS_PARALLELISM=false` to your `.env` to silence the tokenizers fork warning.

The repository's `.gitignore` is updated to exclude `.env`, the virtualenv directory, and common cache files. Never commit secrets.
